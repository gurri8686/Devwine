...
function changeProfilePic() {
 ...
 //        $input['product_image'] = time() . '.' . $file->extension();
 ...
 //        $file->move($destinationPath,$input['product_image']);
 //        $streamingslide1 = "/images/thumbnails/".$input['product_image'];
 //      }
	...
	if(strlen($name)) {
		...
		if(in_array($ext,$valid_formats)) {
			if($size<(1024*1024)) {
				$actual_image_name = $name;//'avatar' .'_'.$userId .'.'.$ext;
...
//print_r($_FILES);
	if(isset($_POST['t']) and $_POST['t'] == "ajax") {
		extract($_POST);		
		$imagePath = 'images/temp/'.$_POST['image_name'];
		$ratio = ($t_width/$w1); 
		$nw = ceil($w1 * $ratio);
		$nh = ceil($h1 * $ratio);
